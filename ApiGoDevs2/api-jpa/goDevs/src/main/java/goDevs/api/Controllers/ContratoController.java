...
public class ContratoController {
    ...
    @PostMapping("/aprovardev/projeto/{projetoid}")
    public  ResponseEntity postDevAprovado(@PathVariable int projetoid){
        Work workContrato = projetoRepositorio.findById(projetoid).get();
        workContrato.setDev_aprovado(true);
        projetoRepositorio.save(workContrato);
        return  ResponseEntity.status(201).build();
    }
    @PostMapping("/rejeitardev/projeto/{projetoid}")
    public  ResponseEntity postDevRejeitado(@PathVariable int projetoid){
        Work workContrato = projetoRepositorio.findById(projetoid).get();
        workContrato.setFk__(null);
        workContrato.setDisponivel(true);
        projetoRepositorio.save(workContrato);
        return  ResponseEntity.status(201).build();
    }
