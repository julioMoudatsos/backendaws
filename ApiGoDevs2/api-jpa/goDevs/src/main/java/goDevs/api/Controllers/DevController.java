...
public class DevController {
    ...
    @PostMapping("/{id}")
    public ResponseEntity attDev(
            @PathVariable int id,
            @RequestBody Contratado novoContratado) {
        Contratado cont = devRepositorio.findById(id).get();
        cont.setCpf(novoContratado.getCpf());
        cont.setEmail(novoContratado.getEmail());
        cont.setExperiencias(novoContratado.getExperiencias());
        cont.setTelefone(novoContratado.getTelefone());
        cont.setQuem_sou(novoContratado.getQuem_sou());
        devRepositorio.save(cont);
        return status(201).build();
    }
